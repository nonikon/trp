...
static void on_tcp_xserver_read(uv_stream_t* stream, ssize_t nread, const uv_buf_t* buf);
...
static int do_protect_fd(int nfd)
{
    ...
    if (sendmsg(chn, &msgh, 0) == -1
        ...
        XLOGE("sendmsg or recv failed: %s.", strerror(errno));
        close(chn);
        return -1;
    }
    close(chn);
...
static void on_tcp_peer_write(uv_write_t* req, int status)
{
    ...
    if (ctx->xserver_blocked && ctx->peer.t.io.write_queue_size == 0) {
        XLOGD("peer write queue cleared.");

        /* peer write queue cleared, start reading from proxy server. */
...
static void on_tcp_xserver_write(uv_write_t* req, int status)
{
    xclient_ctx_t* ctx = req->data;
    io_buf_t* iob = xcontainer_of(req, io_buf_t, wreq);

    if (ctx->peer_blocked && ctx->io_xserver.write_queue_size == 0) {
        XLOGD("proxy server write queue cleared.");

        /* proxy server write queue cleared, start reading from peer. */
...
static void on_tcp_peer_read(uv_stream_t* stream, ssize_t nread, const uv_buf_t* buf)
{
    ...
    if (nread > 0) {
        uv_buf_t wbuf;
        XLOGD("%zd bytes from peer, to proxy server.", nread);
        ...
        xclient.cryptox.encrypt(&ctx->ectx, (u8_t*) wbuf.base, (u32_t) wbuf.len);

        uv_write(&iob->wreq, (uv_stream_t*) &ctx->io_xserver, &wbuf, 1,
            on_tcp_xserver_write);

        if (ctx->io_xserver.write_queue_size > MAX_WQUEUE_SIZE) {
            XLOGD("proxy server write queue pending.");

            /* stop reading from peer until proxy server write queue cleared. */
            uv_read_stop(stream);
            ctx->peer_blocked = 1;
        }
        /* 'iob' free later. */
...
static void on_tcp_xserver_read(uv_stream_t* stream, ssize_t nread, const uv_buf_t* buf)
{
    ...
    if (nread > 0) {
        uv_buf_t wbuf;
        XLOGD("%zd bytes from proxy server, to peer.", nread);
        ...
        uv_write(&iob->wreq, (uv_stream_t*) &ctx->peer.t.io, &wbuf, 1,
            on_tcp_peer_write);

        if (ctx->peer.t.io.write_queue_size > MAX_WQUEUE_SIZE) {
            XLOGD("peer write queue pending.");

            /* stop reading from proxy server until peer write queue cleared. */
            uv_read_stop(stream);
...
static void on_tcp_xserver_connected(uv_connect_t* req, int status)
{
    xclient_ctx_t* ctx = req->data;
...
void start_tcp_forward(xclient_ctx_t* ctx)
{
    ...
    uv_write(&ctx->pending_iob->wreq, (uv_stream_t*) &ctx->io_xserver, &wbuf, 1,
        on_tcp_xserver_write);

    if (xclient.nodelay) {
        /* enable nodelay both proxy server and peer */
...
static void on_udp_io_closed(uv_handle_t* handle)
{
    xclient_ctx_t* ctx = handle->data;

    if (ctx->peer.u.last_iob) {
        xlist_erase(&xclient.io_buffers, xlist_value_iter(ctx->peer.u.last_iob));
    }
    if (ctx->peer.u.npending) {
        u32_t n = ctx->peer.u.npending;
        do {
            xlist_erase(&xclient.io_buffers, xlist_value_iter(
                ctx->peer.u.pending_pkts[--n]));
        } while (n);
    }
    if (ctx->pending_iob) {
        xlist_erase(&xclient.io_buffers, xlist_value_iter(ctx->pending_iob));
...
static int fwd_xserver_udp_packets(xclient_ctx_t* ctx, io_buf_t* iob)
{
    udp_cmd_t* cmd;
    u32_t need;

    if (ctx->peer.u.last_iob) {
        io_buf_t* last_iob = ctx->peer.u.last_iob;
...
static void on_udp_xserver_read(uv_stream_t* stream, ssize_t nread, const uv_buf_t* buf)
{
    xclient_ctx_t* ctx = stream->data;
    io_buf_t* iob = xcontainer_of(buf->base, io_buf_t, buffer);
    ...
    if (nread > 0) {
        ...
        if (fwd_xserver_udp_packets(ctx, iob) != 0) {
        ...
        uv_close((uv_handle_t*) &ctx->io_xserver, on_udp_io_closed);
        /* move 'ctx' node from 'u_xclient_ctxs' to 'xclient_ctxs'. */
        xlist_paste_back(&xclient.xclient_ctxs, xlist_cut(&xclient_pri.u_xclient_ctxs,
...
static void on_udp_xserver_write(uv_write_t* req, int status)
{
    io_buf_t* iob = xcontainer_of(req, io_buf_t, wreq);
...
static void on_udp_xserver_connected(uv_connect_t* req, int status)
{
    xclient_ctx_t* ctx = req->data;
    ...
    if (status < 0) {
        ...
        if (ctx->peer.u.npending) {
            u32_t i = 0;
            XLOGD("write %d pending udp packets.", ctx->peer.u.npending);
            /* write pending udp packets one by one. */
            do {
                io_buf_t* iob = ctx->peer.u.pending_pkts[i++];
                ...
                uv_write(&iob->wreq, (uv_stream_t*) &ctx->io_xserver, &wbuf, 1,
                    on_udp_xserver_write);
            } while (i < ctx->peer.u.npending);

            ctx->peer.u.npending = 0;
        }
        uv_read_start((uv_stream_t*) &ctx->io_xserver, on_iobuf_alloc,
            on_udp_xserver_read);
        /* keepalive with proxy server. */
...
void send_udp_packet(io_buf_t* iob)
{
    ...
    if (xlist_size(&xclient_pri.u_xclient_ctxs) < xclient.n_uconnect) {
        ...
        ctx->peer.u.last_iob = NULL;
        ctx->peer.u.pending_pkts[0] = iob;
        ctx->peer.u.npending = 1;
        ctx->peer_blocked = 0;
        ctx->xserver_blocked = 0;
        ctx->xconnected = 0;
        ctx->stage = 0;
