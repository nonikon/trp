 ...
 */
static const u32 SM4_SBOX_T[256] = {
...
static ossl_inline u32 rotl(u32 a, u8 n)
{
    return (a << n) | (a >> (32 - n));
}

static ossl_inline u32 load_u32_be(const u8 *b, u32 n)
{
    return ((u32)b[4 * n] << 24) |
           ((u32)b[4 * n + 1] << 16) |
           ((u32)b[4 * n + 2] << 8) |
...
static ossl_inline void store_u32_be(u32 v, u8 *b)
{
    b[0] = (u8)(v >> 24);
    b[1] = (u8)(v >> 16);
    b[2] = (u8)(v >> 8);
...
static ossl_inline u32 SM4_T_slow(u32 X)
{
    u32 t = 0;

    t |= ((u32)SM4_S[(u8)(X >> 24)]) << 24;
    t |= ((u32)SM4_S[(u8)(X >> 16)]) << 16;
    t |= ((u32)SM4_S[(u8)(X >> 8)]) << 8;
    t |= SM4_S[(u8)X];
...
static ossl_inline u32 SM4_T(u32 X)
{
    return SM4_SBOX_T[(u8)(X >> 24)] ^
           rotl(SM4_SBOX_T[(u8)(X >> 16)], 24) ^
           rotl(SM4_SBOX_T[(u8)(X >> 8)], 16) ^
           rotl(SM4_SBOX_T[(u8)X], 8);
}

int SM4_set_key(const u8 *key, SM4_KEY *ks)
{
    /*
     * Family Key
     */
    ...
    for (i = 0; i != SM4_KEY_SCHEDULE; ++i) {
        u32 X = K[(i + 1) % 4] ^ K[(i + 2) % 4] ^ K[(i + 3) % 4] ^ CK[i];
        u32 t = 0;
...
void SM4_encrypt(const u8 *in, u8 *out, const SM4_KEY *ks)
{
    u32 B0 = load_u32_be(in, 0);
    u32 B1 = load_u32_be(in, 1);
    u32 B2 = load_u32_be(in, 2);
    u32 B3 = load_u32_be(in, 3);
...
void SM4_decrypt(const u8 *in, u8 *out, const SM4_KEY *ks)
{
    u32 B0 = load_u32_be(in, 0);
    u32 B1 = load_u32_be(in, 1);
    u32 B2 = load_u32_be(in, 2);
