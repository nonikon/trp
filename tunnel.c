...
static void usage(const char* s)
{
    ...
    fprintf(stderr, "  Use section 'tunnel@secname' instead of the default 'tunnel' when parsing config file.\n");
    fprintf(stderr, "\n");
}
...
int main(int argc, char** argv)
{
    ...
    for (i = 1; i < argc; ++i) {
        ...
        if (opt[0] != '-') {
            /* argument only. (opt) */
            snprintf(cfg_sec, sizeof(cfg_sec), "tunnel@%s", opt);
            continue;
        }
